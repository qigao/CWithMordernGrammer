...
## 18. 从 Typed Graph 进入可验证 IR

前十七节已经完成 Graph 的核心转变：Node/Edge 带有类型与 operator/callable knowledge，构造阶段可以 validate 与 propagate type，Graph 成为 typed IR，并且可以 Normalize/Lower 成更适合后续分析与执行的 primitive representation。

统一 IR 的意义确实包括多个 frontend、optimizer、Plan、Reactive 等后续能力，但这些已经各自拥有专章。出版版不在这里提前展开它们，而是直接用第三章的 canonical callables 构造一张 Graph，并据此定义 topology/type/operator/snapshot contract、Lean proof obligation、当前 owning C IR 和 verification/performance evidence。

这样 Chapter 4 只负责回答一个问题：**怎样把多个 typed Callable 之间的计算关系变成可检查、可转换但尚未绑定执行策略的程序数据。**

---

## 19. Canonical Example：把第三章的三个 Callable 连成一张 Graph
...
## 20. Semantic Contract：什么才算一张合法 Typed Graph
...
## 21. Lean / Proof Obligation：Graph 阶段先证明什么
...
## 22. Current C Implementation：Graph 已经是一个真正的 owning IR
...
## 23. Evidence：Graph 这一层已经可以建立完整 C-side verification chain
...
## 24. Performance Evidence：Graph 本身也需要知道自己的成本
