...
Machine transition does not run in send().
...
~~~text
concurrent machine_step count <= 1
~~~

That is the mutable-state isolation Actor provides.
...
Actor requires serialized state mutation.
...
Previously accepted work may still settle.
