...
> ~~~
>
> If the façade changes observable semantics, it is not merely a façade.
...
Stream
    owns/borrows Graph-builder state

Collector
    owns destination policy

Execution
    owns transient frame
~~~
...
~~~text
reducer : (double,int) -> long
expected : (long,int) -> long
    ↓
TYPE_MISMATCH
~~~
...
~~~text
surface accepted
canonical Graph valid
selected backend cannot lower
    ↓
UNSUPPORTED
~~~
