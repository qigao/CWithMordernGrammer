...
Ownership of an accepted task belongs to the Executor until either:
...
There is no hidden queue growth, silent drop, automatic retry, or synchronous fallback.

The caller sees FULL and owns the policy decision.
...
Serial's key invariant is:

~~~text
running <= 1
~~~

It does not imply “one dedicated thread.”
...
Do not accept first and silently cancel later.
