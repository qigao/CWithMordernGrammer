...
Each immutable snapshot can have its own fingerprint so input artifact, rewrite trace, certificate, and compiled plan remain related.
...
No node-id lookup, operator switch, edge resolution, or signature resolution remains in the per-step path.
