...
## 5. FunctionDesc: Describe What a Function Is Before Deciding How to Execute It

A finite signature universe is still useful, but it solves an execution-representation problem. It should not be forced to describe every ordinary C function.
...
If DataBind, TinyMock, Plugin, HTTP/RPC tooling, and diagnostics all need to understand it, the reusable object is descriptive metadata:
...
FunctionDesc therefore does not imply universal dynamic invocation.

A normal function may be described while execution still uses its exact ABI:
...
Only functions admitted by CFlow's execution policy need to project into Callable:
...
One FunctionDesc semantic source can then support DataBind binding, TinyMock, Plugin publication, HTTP/RPC tooling, and diagnostics.

Lean is not needed to prove that a descriptor has no duplicate parameters or only references known types.

Those are generator/compiler checks: unknown type, duplicate parameter, invalid direction, or unsupported ownership should make generation/admission fail.

Lean should appear later for actual semantic laws such as rewrite preservation, associativity, state invariants, or refinement.
...
Therefore **function pointer equality must not be treated as callable semantic equality**. This is the same design pattern as “descriptor address ≠ semantic type identity.”
...
If the bits come from programmer declarations, they are first of all **claims**.
...
but seeing `ASSOCIATIVE` does not by itself prove that arbitrary reassociation is semantically equivalent. Only a semantic law can authorize the rewrite.
...
But if the real C operation involves floating-point rounding, overflow, external state, or error ordering, we must first define observable semantics and then decide whether the theorem applies.
...
Its value is **no hidden allocation, known maximum object size, and predictable ownership**.
