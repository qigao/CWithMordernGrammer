...
Stream mostly lives in Value → Transformation; Reactive adds Time and Demand:

| Concept | Question answered |
|---|---|
| WAIT / Wake | There is no data now; how do we continue later? |
| Scheduler | When and in which scheduling context do we continue? |
| Demand | How many Values may downstream currently accept? |
| Backpressure | How do we keep resources bounded when Producer and Consumer rates differ? |

Reactive therefore does not reinvent a data-processing system. It extends one Typed Graph from synchronous data transformation into execution that is suspendable, resumable, flow-controlled, and explicit about terminal semantics.

One of the most important discoveries here is:

> **advanced asynchronous behavior does not require one giant async Runtime.**

If Publisher, Waitable, Scheduler, Demand, and Subscription remain small, explicit protocols, then synchronous Stream and Reactive can share most computation semantics.
