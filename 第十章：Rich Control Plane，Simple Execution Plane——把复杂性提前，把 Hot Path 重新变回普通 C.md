...
前面 1–13 节已经建立了核心原则：Graph 是分析对象，Normalize/Optimize 发生在 control plane，而最简单的路径应该尽量避免运行时解释。

早期草稿随后又用三十多个小节重复讨论 Plan、Certificate、Proof Trace、Know More / Do Less 与工程哲学。出版稿不再做第二轮概念总结，而直接进入一条真实 pipeline，把这些概念放到同一个 end-to-end execution chain 中观察。

---

## 14. End-to-End Case：canonical pipeline 从 Surface API 走到 Plan
...
## 15. 一个真实 Rewrite：为什么 Metadata Bit 还不够
...
## 15.1 Property claim 只负责 admission
...
## 16. Direct / AOT：最简单的路径应该连 Plan 都不需要
...
## 16.1 Direct eligibility 是删除 runtime abstraction 的资格
...
## 16.2 AOT dispatch 也分多种 authority
...
## 16.4 但当前 Direct 不等于完整 Reduce pipeline
...
## 17. Parallel Reduce：并行是 Execution Refinement，不是 Graph Rewrite
...
## 17.1 Semantic rewrite 与 physical execution refinement 要分开
...
## 17.2 并行 Reduce 需要完整 admission contract
...
## 17.3 Unsupported parallel mode 不能 silent sequential retry
...
## 18. Certificate：Execution artifact 为什么仍然需要被绑定到 Graph
...
## 18.1 Certificate 不是 wire format
...
## 18.2 Version + fingerprint 防止 stale artifact
...
## 18.3 Lean 已经证明 certificate/refinement observation preservation
...
## 19. Proof Trace：为什么还需要记录“发生了哪一次 Rewrite”
...
## 20. Cost Model：Lean 也不应该“证明 benchmark 数字”
...
## 21. Performance Evidence：第十章必须建立 measurement discipline
...
## 21.1 canonical data-flow benchmark 应至少比较
...
## 21.3 不在没有 fresh measurement 时写“X 倍更快”
...
## 22. Machine / Actor 也遵循同一条 Control/Execution Plane 原则

第八、九章看起来与 Graph optimizer 不同，其实原则完全一致。

## 22.1 Machine build 时做复杂工作
...
## 22.2 Actor init/start 时做 capability admission
