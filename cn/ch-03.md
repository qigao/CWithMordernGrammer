...
Graph 或 Stream 在连接两个 Callable 时，首先要求 **f 的 output type 与 g 的 input type compatible**。如果类型关系不成立，错误应该发生在 build/admission，而不是等到运行时拿 `void *` 猜。

### 31.2 Capture 必须拥有明确的生命周期

Lambda / Bind 的核心不是语法糖，而是 **invoke code + captured environment**。
...
因此 **function pointer equality 不能当成 callable semantic equality**。这和“descriptor address ≠ semantic type identity”是同一个设计模式。
...
如果这些 bit 来自程序员声明，那么它们首先只是 **claims**。
...
但不能因为看见 `ASSOCIATIVE` 就自动得出“任意重排都语义等价”。真正授权 rewrite 的必须是 semantic law。
...
但如果真实 C operation 涉及 floating-point rounding、overflow、external state 或 error ordering，就必须先定义 observable semantics，再决定 theorem 是否真的适用。
