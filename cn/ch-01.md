...
也就是说，宏层可以帮助我们减少重复知识，但它不应该偷偷拥有 object runtime、dynamic dispatcher、reflection VM、hidden scheduler 或 automatic ownership system。如果一个抽象需要这些能力，就已经进入了后面的层次，而不再只是本章讨论的 Macro Reuse。

### 15.2 一个事实应该只有一个权威来源

当我们写 `int`、`long`、`float` 时，真正希望表达的是：
...
我们接受一个更小的目标：**finite、explicit、bounded、predictable**。这意味着：
...
我们不需要为了显得“形式化”而写一个 theorem 去证明 `DEFINE_COMPARE(int)` 会被某个具体 compiler 正确预处理。

那不是这里最有价值的 proof obligation。

Lean 真正开始有价值，是下一章出现真正的 semantic question 时，例如：

- 给定有限类型集合，一个 type mapping 是否唯一？
- 给定几个输入类型，推导规则是否 total / deterministic？
- 两个 descriptor 在不同 Translation Unit 中，怎样表示同一个 semantic type？
...
因此应该至少关注 GCC、Clang 和 MSVC / compatible preprocessing mode。这里的目标不是证明所有 compiler 完全相同，而是明确：
