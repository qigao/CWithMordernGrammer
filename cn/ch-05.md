...
### 9.1 Operator semantics 必须与 Graph 一致
...
### 9.2 Stream owns the description, not the source data
...
### 9.3 Evaluation state 必须与 reusable Graph 分离
...
### 11.1 Graph 是 Stream 的核心 owned description

Stream 不是指向一个隐藏 Stream runtime。

它直接拥有 `cflow_graph graph`，operator method 的主要工作就是继续构造它。

这里不是 runtime flow，而是实际 **representation/ownership relation**：Stream 直接 owns Graph description。它不是架构图上的理想关系，而是 data layout 本身。

### 11.2 Range 是输入协议，不是 Graph 的一部分
...
### 11.3 Method fields 是显式 self 的 ISO C11 façade
...
### 11.4 Public Graph view 是 read-only introspection

当前 API 的 `cflow_stream_graph()` 返回 borrowed read-only Graph view。普通用户不应该直接写 `stream.graph.nodes[...] = ...` 来绕过 builder contract。
