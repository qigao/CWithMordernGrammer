...
## 45.1 Plain C Baseline：手写 parser → struct 并没有错
...
## 45.2 第一层：Concrete Parser 只拥有 Syntax
...
## 45.4 第三层：CMeta 提供 Native Semantic Shape
...
## 45.5 第四层：CBind 是 Format-Neutral Decode Kernel
...
## 45.6 Failure Atomicity：绑定失败后 Native Graph 回到 Semantic Zero
...
## 45.7 Boundedness：Serialization 也不能偷偷无限增长
...
## 45.9 Evidence：这个案例怎样证明自己不是“漂亮架构图”
...
## 46.1 RPC Stack 先按 Ownership 分层
...
## 46.2 Method Identity 与 Callable Metadata 也必须分开
...
## 46.3 Params/Result 复用 CSerde，而不是专用 JSON AST API
...
## 46.4 Blocking API 与 Async API 共享协议，但 Ownership 不同
...
## 46.5 JSON-RPC ID 与 Local Request Handle 是两个 Identity
...
## 46.7 No Automatic Retry：RPC 特别不能偷偷 replay
...
## 46.8 Server Registry 也必须 Bounded / Pre-start Admission
...
## 46.9 Exactly-Once Completion：RPC 的另一条核心 invariant
...
## 46.10 Error Surface 必须区分 Domain Failure 与 Transport Failure
...
## 46.11 Evidence：RPC 需要协议、网络、生命周期三层测试
...
## 48.6 Device / UI / Service Runtime
